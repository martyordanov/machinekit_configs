...
def setup_hardware(thread):
    ...
    #hal.Pin('hpg.pwmgen.00.out.01.pin').set(941)
...
# Configure only one extruder, 
# because the E0 mosfet got fucked up
    hal.Pin('hpg.pwmgen.00.out.02.enable').set(True)
    hal.Pin('hpg.pwmgen.00.out.02.value').link('e0-temp-pwm')
    # configure fans
    # configure stepper cooling fan
    hal.Pin('hpg.pwmgen.00.out.05.enable').set(True)
    hal.Pin('hpg.pwmgen.00.out.05.value').set(0.0)
    hal.Pin('hpg.pwmgen.00.out.05.scale').set(255.)
    hal.Pin('motion.analog-out-01').link('f0')
    hal.Pin('hpg.pwmgen.00.out.05.value').link('f0')
    # configure hotend cooling fan
    hal.Pin('hpg.pwmgen.00.out.04.enable').link('exp0-pwm-enable')
    hal.Pin('hpg.pwmgen.00.out.04.value').set(1.0)
    hal.Pin('hpg.pwmgen.00.out.04.value').link('exp0-pwm')
    # configure leds
    # none
    ...
    hal.Pin('hpg.stepgen.01.steppin').set(917)
    hal.Pin('hpg.stepgen.01.dirpin').set(918)
